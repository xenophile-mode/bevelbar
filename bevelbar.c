...
evaluate_args(int argc, char **argv)
{
    ...
    seg_margin = 1;
    seg_size_empty = 7;
    font_str = "Screen-Medium:pixelsize=11";
    basic_colors_str[0] = "#a0a0a0";
    basic_colors_str[1] = "#d6d6d6";
    basic_colors_str[2] = "#535353";

    while ((opt = getopt(argc, argv, "h:v:H:V:b:B:m:e:f:p:o:O:s:c:C")) != -1)
    {
        switch (opt)
        {
    ...
    if (print_config)
    {
        /* Print runtime configuration and exit. Currently, that's only
         * the font height. This allows other programs to adapt to this
         * size. */
        printf("%d\n", font_height);
        return false;
    }
